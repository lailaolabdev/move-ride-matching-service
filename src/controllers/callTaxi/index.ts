...
import festivalPromotionModel, { usingTypeEnum } from "../../models/festivalPromotion";
  ...
  try {
    ...
    if (festivalPromotion && Array.isArray(festivalPromotion)) {
      ...
      for (const promo of festivalPromotion) {
        ...
        const festivalPromotionData = await festivalPromotionModel.findById(promo.promotion);
        ...
        if (festivalPromotionData.usingType === usingTypeEnum.onceTimeType) {
          ...
          const existingUsage = await CallTaxi.findOne({
            passengerId: callTaxi.passengerId,
            'festivalPromotion.promotion': promo.promotion,
            status: { $nin: [STATUS.CANCELED, STATUS.TIMEOUT] }
          });
        ...
        if (festivalPromotionData.usingType === usingTypeEnum.periodType) {
...
export const updateClaimMoneyStatus = async (req: Request, res: Response) => {
  try {
    ...
    await CallTaxi.updateMany({ claimMoney: id }, { $set: { isClaim: true } },);
